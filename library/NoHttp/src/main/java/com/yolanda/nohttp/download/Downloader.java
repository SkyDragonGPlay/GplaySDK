...
public class Downloader {
    ...
    public void download(String what, DownloadRequest downloadRequest, DownloadListener downloadListener) {
            ...
            if (responseCode >= 400) {
                ...
                if (lastFile.exists()) {
                    if (downloadRequest.isDeleteOld())
                        ...
                        downloadListener.onProgress(what, 100, (int)lastFile.length());
                        Logger.d("-------Download finish-------");
                ...
                while (((len = serverStream.read(buffer, 0, 10240)) != -1)) {
                    if (downloadRequest.isCanceled()) {
                        Log.i("NoHttpDownloader", "Download request is canceled.");
                        downloadListener.onCancel(what);
                        break;
                        ...
                        if (contentLength != 0) {
                            //进度精度提升到3000
                            int progress = (int)(count * 3000 / contentLength);
                            if (oldProgress != progress) {
                                oldProgress = progress;
                                downloadListener.onProgress(what, (int)count, (int)contentLength);// 进度通知。
                            }
                        }
                    }
                }
