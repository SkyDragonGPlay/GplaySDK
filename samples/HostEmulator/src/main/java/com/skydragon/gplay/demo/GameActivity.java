...
import com.skydradgon.host.emulator.R;
...
import com.skydragon.gplay.demo.utils.FileUtils;
import com.skydragon.gplay.demo.utils.ThreadUtils;
import com.skydragon.gplay.demo.utils.Utils;
import com.skydragon.gplay.demo.utils.ZipUtils;
...
public class GameActivity extends Activity implements OnPrepareRuntimeListener, IGplayServiceProxy {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Gplay.setProductMode(Gplay.SANDBOX_MODE);
        APP_DATA_DIR = FileUtils.getDataDir(this);
        Utils.ensureFileExist(mCacheDir);
        Utils.ensureFileExist(UNITY_RES_DIR);
    ...
    private static String UNITY_ZIP = "unity_res.zip";
    private static final String UNITY_RES_DIR = FileConstants.getUnityResDir();
    public static final String RUNTIME_FILE_JAR_NAME = "gplay-runtime-jar.jar";

    private static final String UNITY_RUNTIME_NAME = "libunityruntime.jar";
    private static final String UNITY_ENGINE_TYPE = "unity";
    private static final String ENGINE_VERSION = "5.3";
    private static String DIR_STANDARD;
    private static String LIB_UNITY_JAR_PATH;
    private static String HOST_DIFFPATCH_DIR;
    private static String HOST_GAME_SO_DIR;
    private static String COMMON_RESOURCE_DIR;
    private final static String LIB_DIFFPATCH_NAME = "libdiffpatch.so";

    private void initResDir() {
        DIR_STANDARD = mCacheDir + UNITY_ENGINE_TYPE + File.separator + "standards" + File.separator + ENGINE_VERSION + File.separator;
        HOST_DIFFPATCH_DIR = getSystemDataDir() + "diffpatch/";
        HOST_GAME_SO_DIR = getSystemDataDir() + "game/";
        String pathHostRuntimeEngineVersionDir = getSystemDataDir() + "engine" + File.separator
                + UNITY_ENGINE_TYPE + File.separator + ENGINE_VERSION + File.separator;
        LIB_UNITY_JAR_PATH = pathHostRuntimeEngineVersionDir + "javalibrary/" +RUNTIME_FILE_JAR_NAME;
        COMMON_RESOURCE_DIR = pathHostRuntimeEngineVersionDir + "resource/";
        Utils.ensureFileExist(pathHostRuntimeEngineVersionDir + "javalibrary/");
        Utils.ensureFileExist(COMMON_RESOURCE_DIR);
        Utils.ensureFileExist(HOST_DIFFPATCH_DIR);
        Utils.ensureFileExist(HOST_GAME_SO_DIR);
        Utils.ensureFileExist(DIR_STANDARD + "sharelibrary/");
    }

    private void runningHostControll(final IHostControllerListener listener) {
        FileUtils._context = this;
        ...
        ThreadUtils.runAsyncThread(new Runnable() {
            @Override
            public void run() {
                Log.v(TAG, "Rooho#Begin host controll!");
                FileUtils.copyAssetsDataToSD(UNITY_ZIP, UNITY_RES_DIR + UNITY_ZIP);

                ZipUtils.unpackZip(UNITY_RES_DIR + UNITY_ZIP, UNITY_RES_DIR, new ZipUtils.OnUnzipListener() {
                    @Override
                    public void onUnzipSucceed(List<String> unzipFiles) {
                        Log.v(TAG, "Rooho# onUnzipSucceed !" );

                        FileUtils.copyFile(UNITY_RES_DIR + "sharelibrary/", DIR_STANDARD + "sharelibrary/");
                        FileUtils.copyFile(UNITY_RES_DIR + "libgplay.so", DIR_STANDARD + "libgplay.so");
                        FileUtils.copyFile(DIR_STANDARD + "sharelibrary/", HOST_GAME_SO_DIR);
                        FileUtils.copyFile(DIR_STANDARD + "libgplay.so", HOST_GAME_SO_DIR + "libgplay.so");

                        FileUtils.copyFile(UNITY_RES_DIR + "assets/", COMMON_RESOURCE_DIR + "assets/");
                        FileUtils.copyFile(UNITY_RES_DIR + UNITY_RUNTIME_NAME, LIB_UNITY_JAR_PATH);
                        String phoneArch = Utils.getPhoneArch();
                        String soResPath = UNITY_RES_DIR + "diffpatch/" + phoneArch + "/libgplaydiffpatch.so";
                        Log.v(TAG, "Rooho# phone arch   " + phoneArch + " , " + soResPath);

                        FileUtils.copyFile(soResPath, HOST_DIFFPATCH_DIR + LIB_DIFFPATCH_NAME);
                        Log.v(TAG, "Rooho# HOST_DIFFPATCH_DIR   " + HOST_DIFFPATCH_DIR + LIB_DIFFPATCH_NAME + " exist ：" + new File(HOST_DIFFPATCH_DIR + LIB_DIFFPATCH_NAME).exists());
                        Log.v(TAG, "Rooho# LIB_UNITY_JAR_PATH   " + LIB_UNITY_JAR_PATH + " exist ：" + new File(LIB_UNITY_JAR_PATH).exists());


                        FileUtils.delete(UNITY_ZIP);
                        if(listener != null)
                            listener.onSuccess();
                    }
                    ...
                    public void onUnzipProgress(float percent) {
                        ...
                        Log.v(TAG, "Rooho# onUnzipProgress " + percent);
                    }
                ...
                Log.v(TAG, "Rooho#FINISHED host controll!");
            }
        });
    }

    private interface IHostControllerListener {
    ...
    private void startGame() {
        ...
        runningHostControll(new IHostControllerListener() {
            @Override
            public void onSuccess() {
                ThreadUtils.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            mGplayService.startGame(GameActivity.this, jsonGameInfo, GameActivity.this);
                            mGplayService.getGameInfo();
                        } catch(Exception e) {
                            e.printStackTrace();
                        }
                    }
    ...
    protected void onPause() {
        HostEmulator.setHostActivity(null);
        if (mGplayService != null) {
