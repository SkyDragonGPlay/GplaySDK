...
public class CircularProgressButton extends Button {
    ...
    private ColorStateList mChosenColorState;
    ...
    private StateListDrawable mChosenStateDrawable;

    private StateManager mStateManager;
    private State mState;
    private String mIdleText;
    private String mChosenText;
    ...
    public enum State {
        IDLE, CHOSEN, PROGRESS, COMPLETE, ERROR
    }
    ...
    private void initChosenStateDrawable() {
        int colorPressed = getFocusedColor(mChosenColorState);

        StrokeGradientDrawable drawablePressed = createDrawable(colorPressed);
        mChosenStateDrawable = new StateListDrawable();

        mChosenStateDrawable.addState(new int[]{android.R.attr.state_pressed}, drawablePressed.getGradientDrawable());
        mChosenStateDrawable.addState(StateSet.WILD_CARD, background.getGradientDrawable());
    }
    ...
    protected void drawableStateChanged() {
        ...
        } else if (mState == State.CHOSEN) {
            initChosenStateDrawable();
            setBackgroundCompat(mChosenStateDrawable);
        }
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        ...
        try {
            ...
            mChosenText = attr.getString(R.styleable.CircularProgressButton_cpb_textChosen);
            ...
            int chosenStateSelector = attr.getResourceId(R.styleable.CircularProgressButton_cpb_selectorChosen,
                    R.color.cpb_chosen_state_selector);
            mChosenColorState = getResources().getColorStateList(chosenStateSelector);
    ...
    public void morphChosenToIdle() {
        MorphingAnimation animation = createMorphing();
        animation.setFromColor(getNormalColor(mChosenColorState));
        animation.setToColor(getNormalColor(mIdleColorState));
        animation.setFromStrokeColor(getNormalColor(mChosenColorState));
        animation.setToStrokeColor(getNormalColor(mIdleColorState));
        animation.setListener(mIdleStateListener);
        animation.start();
    }

    public void morphIdleToChosen() {
        MorphingAnimation animation = createMorphing();
        animation.setFromColor(getNormalColor(mIdleColorState));
        animation.setToColor(getNormalColor(mChosenColorState));
        animation.setFromStrokeColor(getNormalColor(mIdleColorState));
        animation.setToStrokeColor(getNormalColor(mChosenColorState));
        animation.setListener(mChosenStateListener);
        animation.start();
    }
    ...
    private OnAnimationEndListener mChosenStateListener = new OnAnimationEndListener() {
        @Override
        public void onAnimationEnd() {
            removeIcon();
            setText(mChosenText);
