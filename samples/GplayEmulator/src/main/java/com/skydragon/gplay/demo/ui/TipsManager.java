...
public class TipsManager {
    ...
    public void show(int type) {
        ...
        switch (mode) {
        case MODE_TYPE_LOADING_BACK:
            mDialog.setMessage("确定退出加载吗?");
            break;
        case MODE_TYPE_GAME_BACK:
            mDialog.setMessage("确定退出游戏吗?");
            break;
        case MODE_TYPE_NETWORK_FAILED:
            mDialog.setMessage("连接异常,请检查网络设置后重试.");
            break;
        case MODE_TYPE_INCOMPATIBLE:
            mDialog.setMessage("不兼容,您的版本过低,请升级应用");
            break;
        case MODE_TYPE_MAINTAINING:
            mDialog.setMessage("游戏正在维护中,请稍候.");
            break;
        case MODE_TYPE_INVISIBLE:
            mDialog.setMessage("游戏不可见,获取游戏信息失败!");
            break;
        case MODE_TYPE_GAME_NOT_EXIST:
            mDialog.setMessage("糟糕，游戏资源没加载到，再试试吧！");
            break;
        case MODE_TYPE_FILE_VERIFY_WRONG:
            mDialog.setMessage("文件校验失败！");
            break;
        case MODE_TYPE_ARCH_NOT_SUPPORTED:
            mDialog.setMessage("很抱歉，游戏暂不支持该设备！");
        }
        mDialog.show();
    }
